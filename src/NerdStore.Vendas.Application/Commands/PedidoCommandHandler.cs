...
namespace NerdStore.Vendas.Application.Commands
{
        ...
        IRequestHandler<CancelarProcessamentoPedidoCommand, bool>
    {
        ...
        public async  Task<bool> Handle(AdicionarItemPedidoCommand message, CancellationToken cancellationToken)
        {
            ...
            if (pedido == null)
            {
                pedido = Pedido.PedidoFactory.NovoPedidoRasunho(message.ClienteId);
                pedido.AdicionarItem(pedidoItem);

                _pedidoRepository.Adicionar(pedido);

                // lança evento rascunho iniciado
            ...
            else
            {
                ...
                // lança evento pedido atualizado
                pedido.AdicionarEvento(new PedidoAtualizadoEvent(pedido.ClienteId, pedido.Id, pedido.ValorTotal));
            }

            // lança evento PedidoItemAdicionado
        ...
        private bool ValidarComando(Command message)
        {
            ...
            // senão lançar notificações de erro
            foreach (var error in message.ValidationResult.Errors)
            {
                // 'message.MessageType' é o nome da classe q esta lançando a notificação
                _mediatorHandler.PublicarNotificacao(new DomainNotification(message.MessageType, error.ErrorMessage));

                // 'Neste app de exemplo' a controllerBase vai capturar as notificações
            }
